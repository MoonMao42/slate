//! Brand role API — the 13-method text-role surface (Phase 18 Wave 0).
//!
//! See [`.planning/sketches/MANIFEST.md`] — Phase 18 locks three visual
...
//!   `└─ ★`.
//!
//! Decisions honored: D-01 / D-01a / D-04 / D-05 / D-06 / D-10.
//!
//! Fallback tree per [`RenderMode`]:
//! * `Truecolor` → D-04 blend pill + `38;2;R;G;B` accent fg (brand-anchor
//!   or theme `brand_accent` depending on role).
//! * `Basic` → `› text ‹` + Dim+Bold; no background ANSI.
//! * `None` → plain text, zero `\x1b[` bytes.
//!
//! [`.planning/sketches/MANIFEST.md`]: ../../../.planning/sketches/MANIFEST.md
...
impl<'a> Roles<'a> {
    ...
    // ── Everyday role surfaces — theme-accented (D-01 daily chrome) ────

    /// Command pill (e.g. `slate theme set`) — D-04 blend bg + active
    ...
    // ── Brand-anchor surfaces — fixed #7287fd (D-01) ───────────────────
    ...
    // ── Tree narrative primitives (D-10 — used with println!) ──────────
...
// sketch winners locked in `.planning/sketches/MANIFEST.md`. Run via
// `cargo test --doc brand::roles`.
//
/// Phase 18 sketch canon — the three locked variants.
///
/// ```
...
mod tests {
    ...
    /// Snapshot row `18-W0-roles-truecolor` — command pill emits the D-04

...
//! `~/.config/nvim/` runtimepath. See plans 01-08.
//! Delivered in waves:
...
//! deliverables landed in this file:
...
/// Shape (matches 17- §Pattern 2 loader `PALETTES` entry)
/// leading comment + bare table literal, designed to be spliced into
/// `PALETTES['<variant-id>'] = <output>` by render_loader
/// ```text
...
/// `luafile` this directly. validates each variant's sub-table
...
pub fn render_colorscheme(palette: &Palette, variant_id: &str) -> String {
    ...
    // is self-documenting. preserves this comment when splicing.
...
/// `resolve` tests prove every shipped palette yields a clean
...
// ── Task 2: state-file plumbing ─────────────────────────────

/// Atomically write the active-variant state file observed by the Lua
/// watcher registered by `render_loader` (Task 3).
...
/// `pub(crate)` so adapter and clean helper can
...
-- Per-variant lualine theme tables.  populates; empty stub now.
...
/// 4. (empty in — splices per-variant lualine themes)
...
pub fn render_loader() -> String {
    ...
    // splice one lualine theme table per variant into the
...
// ── Task 2: NvimAdapter — ToolAdapter impl + setup ──────────
...
impl ToolAdapter for NvimAdapter {
    ...
    fn managed_config_path(&self) -> PathBuf {
        // slate writes DIRECTLY to ~/.config/nvim/
...
mod tests {
    ...
    fn render_loader_declares_lualine_themes_table() {
        // ships an EMPTY `LUALINE_THEMES = {}`; fills it.
        let out = render_loader();
        assert!(
    ...
    fn render_loader_calls_nvim_set_hl() {
        // M.load applies groups via the Lua API, never via
        // `:highlight` command strings.
        let out = render_loader();
        assert!(
            out.contains("vim.api.nvim_set_hl"),
            "M.load must call nvim_set_hl per "
        );
    }

    // ── Task 3: lualine theme splice ────────────────────────

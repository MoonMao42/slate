...
///
/// Wave 6 migration: family headings + theme name + description route
...
pub fn handle(_args: &[&str]) -> Result<()> {
    ...
    // same byte contract (sketch 003 + D-01 daily chrome). Graceful
    // degrade per D-05 — plain text when the registry fails to load.
    ...
    for family_name in FAMILY_SORT_ORDER {
        if let Some(themes) = families.get(*family_name) {
            ...
            for theme in themes {
                ...
                // carries the active theme's `brand_accent` (D-01); the
                // description routes through `r.path(...)` for the dim
                // italic treatment.
                print!("{}  ", " ".repeat(2)); // 4 spaces indent per D-16
...
/// theme's `brand_accent` per D-01 daily chrome).
...
// (theme name, description, family heading) was the goal of Wave 6;
...
mod tests {
    ...
    /// Wave 6 chrome contract — family heading carries brand-lavender
    ...
    /// D-01 daily chrome — `theme_name_text` carries the mock theme's
    ...
    /// D-05 graceful degrade — every chrome helper falls back to plain

//! · · Wave-6 integration gate — picker full-preview
...
//! `silent_preview_apply` adapter pipeline (now env-injected per)
...
/// preview call.
/// Centralised so if changes the path layout, only this helper
...
/// VALIDATION row 3 — layer 1 (two-layer ephemeral): navigating in
...
fn picker_nav_does_not_persist_current_file() {
    ...
    assert_eq!(
        after_content.trim(),
        "catppuccin-mocha",
        " layer 1: current file content must not change during navigation"
    );
    assert_eq!(
        before_content, after_content,
        " layer 1: current file byte-for-byte unchanged"
    );
    ...
    assert_eq!(
        ...
        " layer 1: current file mtime must not change during navigation"
    );
}

/// VALIDATION row 5 — layer 1 (triple-guard rollback, active Esc):
...
fn picker_esc_rolls_back_managed_ghostty() {
    ...
    // is broken (regression) and we must surface it here, not
    // silently pass.
    assert!(
    ...
    assert_eq!(
        ...
        " layer 1 (active Esc rollback): managed/ghostty-config must \

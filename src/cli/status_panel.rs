...
pub fn render() -> Result<()> {
    ...
    // D-05 graceful degrade: fall back to plain chrome when the registry
...
/// `Roles` without a live registry (D-06), and so the D-05 graceful
...
fn format_status_panel(
    ...
    // Rounded panel header — the ✦ logo glyph is a brand anchor (D-01),
...
/// Basic, plain otherwise. D-01: brand anchors never depend on the active
...
/// `Roles::heading` (D-01 / Sketch 003 canon).
...
/// Active-theme accent for the theme display name (daily chrome — D-01).
...
mod tests {
    ...
    /// authoritative — same pattern as Wave 2's
    ...
    /// Task 1 Test 3 — D-01 invariant: the `◆ Core Vibe` section heading
    ...
    /// D-05 graceful degrade — when Roles is unavailable (e.g. the theme

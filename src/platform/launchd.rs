...
pub fn generate_plist(binary_path: &str) -> Result<String> {
    ...
    plist_dict.insert(
        "Label".to_string(),
        Value::String(AGENT_LABEL.to_string()),
    );
    ...
    // LaunchEvents with com.apple.notifyd.matching
    let mut notifyd_dict = Dictionary::new();
    notifyd_dict.insert(
        NOTIFICATION_KEY.to_string(),
        Value::Boolean(true),
    );
    
    let mut launch_events_dict = Dictionary::new();
    launch_events_dict.insert(
        "com.apple.notifyd.matching".to_string(),
        Value::Dictionary(notifyd_dict),
    );
    
    plist_dict.insert(
        "LaunchEvents".to_string(),
        Value::Dictionary(launch_events_dict),
    );
    ...
    let plist_content = String::from_utf8(plist_bytes)
        .map_err(|e| SlateError::LaunchdError(format!("Failed to convert plist to UTF-8: {}", e)))?;

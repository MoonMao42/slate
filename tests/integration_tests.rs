...
// Phase 15 `slate demo` integration tests retired in Phase 19 (D-05 / D-06).
...
// in Plan 19-02). The `slate demo` CLI surface + DEMO-02 hint test gates
// are superseded by Phase 19 DEMO-03 integration tests (landing in Wave 4).

/// Phase 19 D-05 CLI smoke: the `slate demo` subcommand has been retired.
...
fn slate_demo_subcommand_is_retired_phase_19() {
    ...
    assert!(
        !output.status.success(),
        "`slate demo` must exit non-zero after Phase 19 retirement"
    );
    let combined = format!(
        "{}{}",

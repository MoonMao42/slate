...
### Fixed
- **Baseline restore**: restoring to the pre-slate baseline no longer immediately re-applies the current theme on top. Rolling back to `baseline-pre-slate` now actually returns your configs to how they were before slate was installed ([#3](https://github.com/MoonMao42/slate/issues/3)).

...
impl ConfigManager {
    ...
    /// slate prints shell exports/source commands directly instead of maintaining env.sh.
    pub fn render_shell_init(&self, shell: &str) -> Result<String> {
        let home = self.base_path.display();
        let current_file = self.current_theme_path().display().to_string();
        let managed_dir = self.base_path.join("managed").display().to_string();

        match shell {
            "zsh" | "bash" => Ok(format!(
                "# slate shell init for {shell}\n\
export SLATE_HOME=\"{home}\"\n\
export SLATE_MANAGED_DIR=\"{managed_dir}\"\n\
if [ -f \"{current_file}\" ]; then\n\
  export SLATE_THEME=\"$(cat \"{current_file}\")\"\n\
fi\n\
# scaffold: adapters append tool-specific exports and source commands.\n"
            )),
            "fish" => Ok(format!(
                "# slate shell init for fish\n\
set -gx SLATE_HOME \"{home}\"\n\
set -gx SLATE_MANAGED_DIR \"{managed_dir}\"\n\
if test -f \"{current_file}\"\n\
    set -gx SLATE_THEME (string trim (cat \"{current_file}\"))\n\
end\n\
# scaffold: adapters append tool-specific exports and source commands.\n"
            )),
...
mod tests {
    ...
    fn test_render_shell_init() {
        ...
        assert!(shell_init.contains("SLATE_HOME"));
        assert!(shell_init.contains("SLATE_THEME"));
    }

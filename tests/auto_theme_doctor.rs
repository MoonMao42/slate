...
fn auto_theme_doctor_reports_fresh_current_legacy_and_changed_launchers_without_writes() {
    ...
    // watcher under SLATE_HOME. Linux requires a discoverable desktop backend;
    // the private stub must never be executed during preparation.
    #[cfg(target_os = "linux")]
    write(
        &td.path().join("bin/gsettings"),
        "#!/bin/sh\nprintf unexpected > \"$HOME/UNEXPECTED\"\nexit 99\n",
        0o700,
    );
    command(td.path())

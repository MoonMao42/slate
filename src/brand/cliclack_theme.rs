...
//!
//! Decisions honored:
//! - **D-08** — override the 9 methods that govern slate-visible framing.
//!   CONTEXT D-08 named `prompt_style` / `error_style`; those do NOT exist
//!   on cliclack 0.5.4's `Theme` trait (see RESEARCH Pitfall 1 — the
//!   authoritative list is encoded here).
//! - **D-01** — intro / outro framing carries the fixed brand lavender
...
//! Phase 18 has no runtime reconfiguration path, so a single call is
//! sufficient.
...
impl Theme for SlateTheme {
    ...
    /// Info bullet — lavender `●` (D-01 brand anchor).

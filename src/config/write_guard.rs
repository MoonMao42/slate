...
impl Drop for Lease {
    fn drop(&mut self) {
        // Close-on-exec does not cover the interval between fork and exec in
        // another thread. Explicitly release our lock even if such a child
        // temporarily holds a duplicate of the same open file description.
        loop {
...
impl ConfigWriteGuard {
    ...
    pub fn acquire(env: &SlateEnv) -> Result<Self> {
        ...
        let guard = Self::adopt_locked(file)?;
        ensure_no_pending_preview(env)?;
        Ok(guard)
    }
    ...
    pub(crate) fn adopt_locked(file: File) -> Result<Self> {
        let lease = Rc::new(Lease { file });
        check_private_file(&lease.file)?;
        let key = file_key(&lease.file)?;
        HELD.with(|held| {
...
mod tests {
    ...
    #[test]
    fn write_guard_drop_unlocks_while_a_duplicate_descriptor_remains_open() {
        let td = tempfile::TempDir::new().unwrap();
        let env = SlateEnv::with_home(td.path().to_owned());
        let guard = ConfigWriteGuard::acquire(&env).unwrap();
        // dup shares the lock's open file description, just like a forked
        // subprocess before close-on-exec has run.
        let duplicate = guard._lease.file.try_clone().unwrap();

//! Pure block renderers for picker preview. (Phase 19 D-07)
//!
...
//!
//! Migrated verbatim from `src/cli/demo.rs` in Plan 19-02 (Wave 1). The 4
//! Phase 15 block renderers (code / tree / git-log / progress) plus the
//! new `render_palette_swatch` (D-03) are consumed by
//! `src/cli/picker/preview/compose.rs` (Plan 19-04) to assemble the
...
mod tests {
    ...
    /// Plan 19-04 compose.rs is its replacement).
